...
public class CustomerDto implements Validator {
    ...
    @Pattern(regexp = "[0-9]{9,12}",message ="*Length must be 9-12 character and only include number!!" )
    ...
    public void validate(Object target, Errors errors) {
        ...
        if(!"".equals(customerDto.getBirthDay())){
            ...
            if((LocalDate.now().minusYears(18)).isBefore(date)){
                errors.rejectValue("birthDay","date.valid","Customer birthday must be before today!");
            }
        }
        ...
        if(customerDto.getPhoneList().contains(customerDto.getPhone())){
            errors.rejectValue("phone","phone.duplicate","This phone number already exists");
        }
        ...
        if(customerDto.getEmailList().contains(customerDto.getEmail())){
            errors.rejectValue("email","email.duplicate","This email already exists");
        }
